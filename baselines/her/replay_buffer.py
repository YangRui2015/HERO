...
class ReplayBuffer:
    def __init__(self, buffer_shapes, size_in_transitions, T, sample_transitions, info=None): 
        ...
        # std
        self.buffer_std = np.empty(self.size)
        if type(self.info) == dict:
            self.info['std_vertical'] = True
    ...
    def sample(self, batch_size):
        ...
        self.info['std'] = self.buffer_std[:self.current_size]
    ...
    def store_episode(self, episode_batch):
        ...
        with self.lock:
            ...
            self.buffer_std[idxs] = np.std(episode_batch['ag'], axis=1).sum(axis=1)
